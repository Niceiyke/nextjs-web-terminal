...
// Create connection
export async function createConnection(userId: string, input: ConnectionInput): Promise<Connection | null> {
  ...
  const encryptedKeyContent = input.private_key_content ? encrypt(input.private_key_content) : null;
...
export async function updateConnection(
  ...
  if (input.private_key_content !== undefined) {
    updates.private_key_content = input.private_key_content ? encrypt(input.private_key_content) : null;
  }
...
export async function getDecryptedConnection(userId: string, id: string): Promise<ConnectionInput | null> {
  const conn = await getConnection(userId, id);
  if (!conn) return null;

  // Decrypt SSH keys if present
  let decryptedKeys: SSHKey[] | undefined;
  if (conn.ssh_keys && Array.isArray(conn.ssh_keys) && conn.ssh_keys.length > 0) {
  ...
  return {
    ...
    private_key_content: conn.private_key_content ? decrypt(conn.private_key_content) : undefined,
